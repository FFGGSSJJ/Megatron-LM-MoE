...
def _sssglu_bwd_kernel(
    ...
    # d/da ssslu(a) = gate(a) + a * gate'(a)
    denom = 1.0 + tl.abs(a)
    gate_a = 0.5 + 0.5 * a / denom
    ssslu_a = gate_a * a
    d_ssslu = gate_a + 0.5 * a / (denom * denom)

    if HAS_PROBS:
        p = tl.load(p_ptr + rm, mask=rm < M, other=0.0).to(tl.float32)
        gy_scaled = gy * p[:, None]
        # partial grad_probs over this D-block: sum_d (ssslu(a) * b * gy)
        partial = tl.sum(tl.where(mask, ssslu_a * b * gy, 0.0), axis=1)
        tl.atomic_add(gp_ptr + rm, partial, mask=rm < M)
    else:
        gy_scaled = gy

    grad_a = gy_scaled * d_ssslu * b
    grad_b = gy_scaled * ssslu_a
...
def sssglu_forward(
    ...
    y = ssslu(a) * b, optionally scaled by per-row `probs`.
    """
