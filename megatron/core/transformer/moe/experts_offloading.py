...
class OffloadingExpertsGroupedSwiMLP(torch.autograd.Function):
    ...
    def _grouped_gemm(
        ...
        trans_a: bool,
        trans_b: bool,
        compute_streams: torch.cuda.Stream,
        ...
        b_cunks = list(torch.unbind(b, dim=0)) if isinstance(b, torch.Tensor) else list(b)

        with torch.cuda.stream(compute_streams):
            for i in range(num_local_experts):
                torch.mm(
                    a_chunks[i].t() if trans_a else a_chunks[i],
                    b_cunks[i].t() if trans_b else b_cunks[i],
                    out=c[i]
                )
    ...
    def call_forward_a(
        ...
        for chunk_idx in range(config.moe_offloading_num_chunks):
            ...
            if chunk_idx + 1 < config.moe_offloading_num_chunks:
                next_buffer_metadata = cls._prefetch_expert_weights(chunk_idx + 1, cpu_w1, gpu_w1_buffers, stream_manager, config)
            ...
            OffloadingExpertsGroupedSwiMLP._grouped_gemm(
    ...
    def call_forward_y(
        ...
        for chunk_idx in range(config.moe_offloading_num_chunks):
            # prefetch the next chunk of expert weights to GPU buffer
            if chunk_idx + 1 < config.moe_offloading_num_chunks:
                next_buffer_metadata = cls._prefetch_expert_weights(chunk_idx + 1, cpu_w2, gpu_w2_buffers, stream_manager, config)
