"""Triton kernels for RLGLU activation with optional per-row probability scaling.
...
def _rlglu_fwd_kernel(
    ...
    # gate(a) = relu(a) - 0.5 * ln(1 + |a|); y = gate(a) * b
    gate_a = tl.maximum(a, 0.0) - 0.5 * tl.log(1.0 + tl.abs(a))
    y = gate_a * b
...
def rlglu_forward(
    ...
    """Triton RLGLU forward.

    y = gate(a) * b, gate(a) = relu(a) - 0.5*ln(1+|a|), optionally scaled by per-row `probs`.
    """
