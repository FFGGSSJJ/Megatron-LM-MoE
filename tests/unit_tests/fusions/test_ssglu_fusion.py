"""Tests for the fused SSGLU kernels (``fused_bias_ssglu.py``).

SSGLU is SwiGLU with the sigmoid inside SiLU replaced by softsign rescaled to (0, 1). The
...
def _ref(x, bias=None):
    """Pure-torch autograd reference: sslu(y1) * y2 with sslu(y) = y*(0.5+0.5*softsign(y))."""
    if bias is not None:
        x = x + bias
...
def test_bias_ssglu_against_autograd_reference(input_dtype, use_bias):
...
def test_weighted_bias_ssglu(input_dtype):
    ...
    y = bias_ssglu_impl(x, None) * weights
    y = y.to(input_dtype)
    ...
    y_2 = weighted_bias_ssglu_impl(x_2, None, weights_2)
