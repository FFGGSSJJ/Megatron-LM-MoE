...
class TestMoELayerFP16:
    ...
    @pytest.mark.parametrize("moe_token_dispatcher_type", ["allgather", "alltoall"])
    @pytest.mark.parametrize("num_moe_experts", [64, 128])
    @pytest.mark.parametrize("tp_size,ep_size", [(1, 1)])
    @staticmethod
    def test_legacy_moe_layer_fp16_forward_backward(
        num_moe_experts, moe_token_dispatcher_type, tp_size, ep_size
    ):
        """Test MoE layer forward and backward pass with fp16 params and inputs."""
        Utils.initialize_model_parallel(
            tensor_model_parallel_size=tp_size, expert_model_parallel_size=ep_size
        ...
        moe_layer.experts.weight1.main_grad = torch.zeros(moe_layer.experts.weight1.shape, device=moe_layer.experts.weight1.device, dtype=torch.float32)
        moe_layer.experts.weight2.main_grad = torch.zeros(moe_layer.experts.weight2.shape, device=moe_layer.experts.weight2.device, dtype=torch.float32)
